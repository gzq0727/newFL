...
//port hash table:used to record data transmitted
struct port_hash_table {
    uint32_t port_key;
    uint64_t data_size;
    UT_hash_handle hh;
};
struct port_hash_table *port_tables = NULL;

//new form of hash table:per flow
struct flowlet_hash_table {
    uint32_t key;
    uint32_t last_output_port;
    uint64_t packet_count;
    uint64_t flowlet_count;
    uint64_t current_size; //统计flowlet的大小
    struct timeval t_val;
    struct timeval lst_fl_time;
    uint64_t start_path_size; // flowlet第一个包时当前链路传输的数据总量
    uint64_t end_path_size; // flowlet最后一个包时当前链路传输的数据总量
    UT_hash_handle hh;
};
struct flowlet_hash_table *flowlet_hash_tables = NULL;

//new form of hash table:per datapath
struct dp_hash_table{
    uint32_t br_key;
    uint64_t packet_count;
    uint64_t flowlet_count;
    UT_hash_handle hh;
};
struct dp_hash_table *hash_tables = NULL;
...
static
struct flowlet_hash_table *get_last_time(uint32_t hash_key){
    struct flowlet_hash_table *s;
    pthread_rwlock_rdlock(&lock1); 
    HASH_FIND_INT(flowlet_hash_tables, &hash_key, s);
    pthread_rwlock_unlock(&lock1);

    return s;
}

static struct flowlet_hash_table 
*update_last_time_hash (uint32_t hash_key, struct timeval current_time){
    struct flowlet_hash_table *s;
    s = get_last_time(hash_key);
    if(s == NULL){
        s = malloc(sizeof(struct flowlet_hash_table));
	memset(s,0,sizeof(struct flowlet_hash_table));
        s->key = hash_key;
	...
	s->flowlet_count = 0;
	s->current_size = 0;
        s->lst_fl_time.tv_sec = current_time.tv_sec;
        s->lst_fl_time.tv_usec = current_time.tv_usec;
	s->start_path_size = 0;
	s->end_path_size = 0;
        pthread_rwlock_wrlock(&lock1);
        HASH_ADD_INT(flowlet_hash_tables, key, s );
	pthread_rwlock_unlock(&lock1);
    }
    else{
...
static
struct flowlet_hash_table *get_last_output_port(uint32_t hash_key){
    struct flowlet_hash_table *s;
    pthread_rwlock_rdlock(&lock1); 
    HASH_FIND_INT(flowlet_hash_tables, &hash_key, s);
    pthread_rwlock_unlock(&lock1);
...
static 
struct port_hash_table *get_port_hash_table(uint32_t port_key){
    struct port_hash_table *s;

    pthread_rwlock_rdlock(&lock1);
    HASH_FIND_INT(port_tables, &port_key,s);
    pthread_rwlock_unlock(&lock1);

    if(!s){
        s = malloc(sizeof(struct port_hash_table));
        memset(s,0,sizeof(struct port_hash_table));
	s->port_key = port_key;
        s->data_size = 0;

        pthread_rwlock_wrlock(&lock1);
	HASH_ADD_INT(port_tables, port_key, s);
        pthread_rwlock_unlock(&lock1);
    }
    return s;
}
...
/*
static void
update_port_hash_table(char *bridge_name, uint32_t port, uint64_t data_size){
    struct port_hash_table *r;
    r = get_port_hash_table(bridge_name, port);
    r->data_size = data_size;
}
*/

static
struct dp_hash_table *get_dp_hash_table (uint32_t br_hash_key){
    struct dp_hash_table *s;
    pthread_rwlock_rdlock(&lock1); 
    HASH_FIND_INT(hash_tables, &br_hash_key, s);
    pthread_rwlock_unlock(&lock1);
    return s;
}

static
struct dp_hash_table *add_dp_hash_table (uint32_t br_hash_key){
    struct dp_hash_table *s;
    s = malloc(sizeof(struct dp_hash_table));
    memset(s,0,sizeof(struct dp_hash_table));
    s->packet_count = 0;
    s->flowlet_count = 0;
    s->br_key = br_hash_key;
    //strncpy(s->br_name,bridge_name,10);

    pthread_rwlock_wrlock(&lock1);
    HASH_ADD_INT(hash_tables, br_key, s);
    pthread_rwlock_unlock(&lock1);
...
struct ofputil_bucket *get_flowlet_random_bucket(struct group_dpif *group){
    ...
    memset(head,0,sizeof(struct bucket_gzq));
    ...
    LIST_FOR_EACH (bucket, list_node, &group->up.buckets){
        ...
        memset(new_bucket,0,sizeof(struct bucket_gzq));
...
static uint32_t
cal_br_hash_key (char *bridge_name){
    int i = 0;
    uint32_t key = 0;
    
    while(bridge_name[i] != '\0'){
	key = key + (i+13) * bridge_name[i]; 
	i = i + 1;
    }
...
static uint32_t
cal_port_hash_key (uint32_t br_hash_key, uint32_t port){
    uint32_t key = 0;
    key = br_hash_key*7 + 11 * port; 
    
    return key;
}

static uint32_t
cal_flowlet_hash_key(uint32_t br_hash_key, uint32_t flow_key){
    uint32_t key = 0;
    key = br_hash_key*7 + 11 * flow_key;
...
{
    if(fp == NULL){
        if((fp = fopen("/home/tank/flowlet_log.txt", "a+")) == NULL) {
                printf("open log file failed!");
                exit(0);
            }
    }
    //fprintf(fp,"run this function\n");
    //fflush(fp);

    //the main logic of flowlet switching
    uint32_t inport;
    uint32_t ip_src;
    uint32_t ip_dst;
    uint16_t tcp_src;
    ...
    int fd;
    int offset = 0;
    uint32_t br_hash_key = 0;
    uint32_t port_hash_key = 0;

    //char *datapath_name;
    struct dp_hash_table *datapath_hash_table;

    //datapath_name = ctx->xbridge->name;	
    strcpy(datapath_name, ctx->xbridge->name);
    br_hash_key = cal_br_hash_key(datapath_name);
    ...
    datapath_hash_table = get_dp_hash_table(br_hash_key);
    if( datapath_hash_table == NULL){
        datapath_hash_table = add_dp_hash_table(br_hash_key);
    }
    
    //record the packet processed by datapath
    datapath_hash_table->packet_count = datapath_hash_table->packet_count + 1;
    ...
    uint32_t key = cal_flowlet_hash_key(br_hash_key, hash_func(inport, ip_src, ip_dst, tcp_src, tcp_dst));
    offset = sprintf(buf, "%s flow_hash_key %32u\n",datapath_name,key);
    //fprintf(fp, "%s flow_hash_key %32u\n",datapath_name,key);
    //fflush(fp);
    ...
    if(!ctx->xin->packet){
	offset += sprintf(buf+offset,"packet is null\n");
    }

    offset += sprintf(buf+offset, "%s flow_hash_key %32u: currnet time %ld %ld\n",datapath_name,key,current_time.tv_sec,current_time.tv_usec);
    //fprintf(fp, "%s flow_hash_key %32u: currnet time %ld %ld\n",datapath_name,key,current_time.tv_sec,current_time.tv_usec);
    //fflush(fp);    

    //2.get last time for flow
    struct flowlet_hash_table *lt;
    struct timeval last_time;
    lt = get_last_time(key);
    offset += sprintf(buf+offset,"%s flow_hash_key %32u: packet_size %" PRIu32 "\n",datapath_name,key,ctx->xin->packet ? ctx->xin->packet->size_ : 58);
    //fprintf(fp,"%s flow_hash_key %32u: packet_size %" PRIu32 "\n",datapath_name,key,ctx->xin->packet->size_);

    struct ofputil_bucket *bucket; //the bucket need to be excuted
    uint32_t output_port;

    if (lt == NULL) { //new flow
        offset += sprintf(buf+offset, "%s flow_hash_key %32u: new_flow\n",datapath_name,key);
        //fprintf(fp, "%s flow_hash_key %32u: new_flow\n",datapath_name,key);
	//fflush(fp);

        lt = update_last_time_hash(key, current_time);//add time for new flowlet to hash table
        //update flowlet information for current flow
	lt->flowlet_count = 1;       
        datapath_hash_table->flowlet_count = datapath_hash_table->flowlet_count + 1;
	//fprintf(fp,"4084\n");
	//fflush(fp);
	
	lt->current_size = ctx->xin->packet ? ctx->xin->packet->size_ : 58;
        //lt->current_size = ctx->xin->packet->size_;
	...
	//fflush(fp);
        ...
        //更新出端口统计信息
        port_hash_key = cal_port_hash_key(br_hash_key,output_port);
	//fprintf(fp,"4100\n");
	//fflush(fp);
	struct port_hash_table *p;
	p = get_port_hash_table(port_hash_key);
	//fprintf(fp,"4104\n");
	//fflush(fp);
	p->data_size = p->data_size + ctx->xin->packet ? ctx->xin->packet->size_ : 58;
	//fprintf(fp,"run here1\n");
	//fflush(fp);

	//update congestion degree needed information
	lt->lst_fl_time.tv_sec = current_time.tv_sec;
	lt->lst_fl_time.tv_usec = current_time.tv_usec;
	lt->start_path_size = p->data_size;  //记录该flowlet第一个包时当前链路传输的数据总量
	lt->end_path_size = p->data_size;  //记录该flowlet最后一个包时当前链路传输的数据总量
    } else {//not a new flow
        offset += sprintf(buf+offset, "%s flow_hash_key %32u: not_a_new_flow\n",datapath_name,key);
        //fprintf(fp, "%s flow_hash_key %32u: not_a_new_flow\n",datapath_name,key);
	//fflush(fp);
        ...
        offset += sprintf(buf+offset, "%s flow_hash_key %32u: diff_time %64lu\n",datapath_name,key,time_diff);

        //record new lasttime for flowlet
        update_last_time_hash(key, current_time);
	
	//fprintf(fp,"update_last_time_hash not a new flow\n");
	//fflush(fp);
        //3.if timeout is occured, random choose one output port and record
        if (time_diff >= timeout) {//trigger a new flowlet
            offset += sprintf(buf+offset, "%s flow_hash_key %32u: new_flowlet\n",datapath_name, key);
            //fprintf(fp, "%s flow_hash_key %32u: new_flowlet\n",datapath_name, key);
	    //fflush(fp);
	    ...
	    p->data_size = p->data_size + ctx->xin->packet ? ctx->xin->packet->size_ : 58;
            ...
            //计算congestion degree
	    uint64_t time_interval = diff_time(last_time, lt->lst_fl_time);
	    uint64_t other_data_size = lt->end_path_size - lt->start_path_size -  lt->current_size;
            double congestion_degree = time_interval ? other_data_size/(double)time_interval/131072000000 : 0; // *8/1000000/1024/1024 = Mb/s
            offset += sprintf(buf+offset, "%s flow_hash_key %32u: last_flowlet_output_port %32u congestion_degree %lf last_flowlet_size %" PRId64 "\n",datapath_name, 
                key,lt->last_output_port,congestion_degree,lt->current_size);
	    ...
	    lt->current_size = ctx->xin->packet ? ctx->xin->packet->size_ : 58;
	    lt->start_path_size = p->data_size; 
	    lt->end_path_size = p->data_size;
	    
            //更新flowlet的last output port
            update_last_output_port_hash(key, output_port);
	    //fprintf(fp,"a new flowlet\n");
	    //fflush(fp);

        } else {//not trigger a new flowlet
            offset += sprintf(buf+offset, "%s flow_hash_key %32u: old_flowlet\n",datapath_name, key);
            //fprintf(fp, "%s flow_hash_key %32u: old_flowlet\n",datapath_name, key);
            //fflush(fp);
	    
	    //lt->current_size = lt->current_size + ctx->xin->packet->size_;
	    if(!ctx->xin->packet){
	        //fprintf(fp,"pacekt is null\n");
	        //fflush(fp);
	    }
	    lt->current_size = ctx->xin->packet ? lt->current_size + ctx->xin->packet->size_ : lt->current_size + 58;
	    ...
	    lt->end_path_size = p->data_size;
	    //fprintf(fp,"get flowlet last bucket\n");
 	    //fflush(fp);
        }
    }
    //flowlet statistics
    offset += sprintf(buf+offset, "%s flow_hash_key %32u: flowlet_current_size %" PRId64 "\n",datapath_name,key,lt->current_size);
    offset += sprintf(buf+offset, "%s flow_hash_key %32u: output_port %32u\n",datapath_name,key,output_port);

    //flow statistics
    lt->packet_count = lt->packet_count + 1;
    offset += sprintf(buf+offset, "%s flow_hash_key %32u: packet_count %" PRId64 "\n",datapath_name,key,lt->packet_count);
    offset += sprintf(buf+offset, "%s flow_hash_key %32u: flowlet_count %" PRId64 "\n",datapath_name,key,lt->flowlet_count);

    //datapath statistics
    offset += sprintf(buf+offset, "%s packet_count %" PRId64 "\n",datapath_name,datapath_hash_table->packet_count); 
    offset += sprintf(buf+offset, "%s flowlet_count %" PRId64 "\n\n",datapath_name,datapath_hash_table->flowlet_count);
    //fprintf(fp, "%s packet_count %" PRId64 "\n",datapath_name,datapath_hash_table->packet_count); 
    //fflush(fp);
    ...
    //file lock
    fd = fileno(fp);
    //fprintf(fp,"fd = fileno(fp) \n");
    //fflush(fp);

    fcntl(fd, F_SETLKW, file_lock(F_WRLCK, SEEK_SET));
    //fprintf(fp,"fctl get lock\n");
    //fflush(fp);
    fprintf(fp, "%s",buf );
    fflush(fp);
    fcntl(fd, F_SETLK, file_lock(F_UNLCK, SEEK_SET));
    free(buf);
    //fprintf(fp,"drop lock\n\n");
    //fflush(fp);
}
//mod end by gzq
